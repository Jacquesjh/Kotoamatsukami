...
class HandDetector:
    ...
    def __init__(
        self,
        max_hands: int = 2,
        detection_con: float = 0.8,
        model_complexity: int = 1,
        min_track_con: float = 0.2,
        normalize: bool = True,
    ) -> None:
    ...
    def find_hands(
        self, image: np.ndarray, draw_marks: bool = True, draw_box: bool = True
    ) -> tuple:
    ...
    def _get_hands(
        ...
        for hand_type, hand_landmarks in zip(
            ...
            hand = dict()
            unpack = self._get_landmarks(
                hand_landmarks=hand_landmarks, height=height, width=width
            )
